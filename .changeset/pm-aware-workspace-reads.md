...
- `parsePackageJsonWorkspaces` previously did unchecked casts on a present `workspaces` field: a malformed value (`"workspaces": 42`, `null`, a `catalog` that is a string instead of an object, etc.) silently degraded to `{}` — an unreadable manifest read as "no workspaces," the exact silent-empty-manifest failure mode this release exists to eliminate. It now validates the shape and fails with `CatalogAssemblyError` naming what was wrong. An absent `workspaces` field still correctly yields `{}`.
