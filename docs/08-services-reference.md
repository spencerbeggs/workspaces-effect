...
The reader is exported: `parsePackageJsonWorkspaces(content)` yields the `PackageJsonWorkspaces` slice (`packages`, `catalog`, `catalogs`), and `catalogSetFromPackageJson(content)` yields a `CatalogSet` built from it. Both fail with `CatalogAssemblyError` when the text is not valid JSON, or when a present, non-null `workspaces` field is malformed (wrong type, or an object whose `packages`/`catalog`/`catalogs` have the wrong shape) — which means a **malformed root `package.json` in a repo with no `pnpm-workspace.yaml` now fails the read** instead of being silently ignored. An absent or explicitly `null` `workspaces` field is not an error; it yields an empty result, same as a repo with no workspaces declared at all.
...
| `parsePackageJsonWorkspaces(content)` | `Effect<PackageJsonWorkspaces, CatalogAssemblyError>` | Parse the `workspaces` field out of raw `package.json` text. Accepts the array form (globs only) and the object form. Every field is `undefined` when there is no `workspaces` field, or when it is explicitly `null` |
...
Both fail with `CatalogAssemblyError` (`source: "manifest"`) when `content` is not valid JSON, or when a present, non-null `workspaces` field is malformed — the wrong type entirely, or an object form whose `packages`/`catalog`/`catalogs` have the wrong shape (e.g. `packages` not an array, `catalog` not a string record). An absent or `null` `workspaces` field is not an error and yields an empty result. Unknown/extra keys under an object-form `workspaces` are ignored rather than rejected, so tooling-specific keys like yarn's `nohoist` do not fail the read.
